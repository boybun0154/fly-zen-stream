...
const formatTime = (iso: string) => new Date(iso).toLocaleString([], { weekday: "short", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });
...
function Checkout() {
  ...
  const [contact, setContact] = useState({ email: "", phone: "", card: "", expiry: "", cvc: "", name: "" });
    ...
    <main className="min-h-screen bg-background pt-24 pb-24">
      <section className="mx-auto max-w-6xl px-6 md:px-12">
        <div className="animate-fade-up">
          <Link to="/" className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground hover:text-foreground">← Home</Link>
        ...
        <div className="mt-12 grid grid-cols-1 gap-12 lg:grid-cols-[1.6fr_1fr]">
          <form onSubmit={handlePay} className="animate-fade-up space-y-12" style={{ animationDelay: "0.1s" }}>
            <FormSection title="Contact" step="01">
              <Input label="Email" type="email" required value={contact.email} onChange={(v) => setContact({ ...contact, email: v })} placeholder="you@example.com" />
              <Input label="Phone" type="tel" required value={contact.phone} onChange={(v) => setContact({ ...contact, phone: v })} placeholder="+1 555 000 0000" />
            </FormSection>

            <FormSection title="Payment" step="02">
              <Input label="Card number" required value={contact.card} onChange={(v) => setContact({ ...contact, card: v })} placeholder="1234 5678 9012 3456" />
              <div className="grid grid-cols-2 gap-4">
                <Input label="Expiry" required value={contact.expiry} onChange={(v) => setContact({ ...contact, expiry: v })} placeholder="MM / YY" />
                <Input label="CVC" required value={contact.cvc} onChange={(v) => setContact({ ...contact, cvc: v })} placeholder="123" />
              </div>
              <Input label="Cardholder name" required value={contact.name} onChange={(v) => setContact({ ...contact, name: v })} placeholder="Name on card" />
            </FormSection>

            <button type="submit" disabled={submitting} className="w-full rounded-full bg-foreground py-5 text-xs font-semibold uppercase tracking-[0.3em] text-background transition hover:opacity-90 disabled:opacity-50">
          ...
          <aside className="animate-fade-up lg:sticky lg:top-28 lg:self-start" style={{ animationDelay: "0.2s" }}>
            <div className="rounded-2xl border border-border bg-card p-6">
              ...
              <div className="mt-6 space-y-4 border-t border-border pt-4">
                <SummaryLine label={`${primary.originCode} → ${primary.destinationCode}`} detail={`${formatTime(primary.departTime)} · ${primary.airline}`} value={`$${(primary.price * state.passengerCount).toLocaleString()}`} sub={`${state.passengerCount} × $${primary.price}`} />
                {secondary && (
                  <SummaryLine label={`${primary.destinationCode} → ${secondary.cityCode}`} detail={`Connection · ${secondary.city}`} value={`$${(secondary.price * state.passengerCount).toLocaleString()}`} sub={`${state.passengerCount} × $${secondary.price}`} />
                )}
              </div>

              <div className="mt-4 space-y-2 border-t border-border pt-4">
                <p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Passengers</p>
                {state.passengers.map((p, i) => {
                  const a = state.addons[i];
                  const tier = inferTier(state.seats[i]);
                  return (
                    <div key={p.id} className="flex justify-between text-xs text-muted-foreground">
                      <span>{p.firstName || `P${i + 1}`} · Seat {state.seats[i] ?? "—"}</span>
                      <span>${((a.checkedBag ? ADDON_PRICES.checkedBag : 0) + (a.priority ? ADDON_PRICES.priority : 0) + (tier ? SEAT_TIER_PRICE[tier] : 0)).toLocaleString()}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mt-6 space-y-2 border-t border-border pt-4 text-sm">
                ...
                {totals.savings > 0 && <Line label="Deal savings" value={`−$${totals.savings.toLocaleString()}`} accent />}
              </div>
...
function SummaryLine({ label, detail, value, sub }: { label: string; detail: string; value: string; sub: string }) {
  return (
