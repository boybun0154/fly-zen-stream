  ...
  head: () => ({
    meta: [
      { title: "KimFlights — Fly Beyond" },
      {
        name: "description",
        content:
          "A cinematic flight booking experience. Search, discover deals, and curate journeys.",
      },
    ],
  }),
  component: Index,
...
function Index() {
  ...
  const onSearch = (e: React.FormEvent) => {
    ...
    navigate({
      to: "/results",
      search: { origin, destination, date, passengers, adults, children },
    });
  };
          ...
          <div className="grid grid-cols-1 gap-px overflow-hidden rounded-xl md:grid-cols-[1fr_1fr_1fr_auto_auto]">
            ...
            <label className="flex flex-col gap-1 bg-background/60 px-5 py-4 transition-all duration-300 ease-in-out cursor-pointer hover:bg-background/80 focus-within:bg-background">
              ...
              <input
                ...
                className="w-full bg-transparent text-sm font-medium text-foreground outline-none [color-scheme:dark]"
              />
            </label>
            <PassengerPicker
              adults={adults}
